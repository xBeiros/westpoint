...
import { edit as editRoute, destroy, togglePublish as togglePublishRoute, store, update } from '@/routes/ucp/wiki';
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                                    ...
                                    <div class="relative">
                                        ...
                                        <div class="relative">
                                            <input
                                                ...
                                                @focus="handleFocus"
                                                @blur="handleBlur"
                                                class="w-full px-4 py-2 pr-10 bg-background border border-sidebar-border/70 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary"
                                                placeholder="Kategorie auswählen oder neu erstellen"
                                            />
                                            <button
                                                type="button"
                                                @mousedown.prevent="toggleCategoryDropdown"
                                                class="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground hover:text-foreground transition-colors"
                                            >
                                                <svg 
                                                    class="w-4 h-4"
                                                    fill="none" 
                                                ...
                                                >
                                                    <path 
                                                        ...
                                                        stroke-width="2" 
                                                        :d="showCategoryDropdown ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7'" 
                                                    />
                                                </svg>
                                            </button>
                                            ...
                                            >
                                                ...
                                                <template v-if="existingCategories.length > 0">
                                                    <div class="px-3 py-2 text-xs font-semibold text-muted-foreground uppercase border-b border-sidebar-border/70">
                                                        Existierende Kategorien
                                                    </div>
                                                    <div 
                                                ...
                                                <div 
                                                    v-if="categoryInput.trim() && !existingCategories.some(c => c.toLowerCase() === categoryInput.trim().toLowerCase())"
                                                    @mousedown.prevent="createNewCategory"
                                                    class="px-4 py-2 hover:bg-muted/50 cursor-pointer transition-colors border-t border-sidebar-border/70 bg-primary/5"
                                                >
            ...
            <Dialog v-model:open="isEditModalOpen">
                <DialogScrollContent class="max-w-4xl">
                    <DialogHeader>
                        <DialogTitle class="text-2xl font-semibold">Artikel bearbeiten</DialogTitle>
                        <DialogDescription>
                            Bearbeite den Wiki-Artikel mit Titel, Beschreibung und Inhalt.
                        </DialogDescription>
                    </DialogHeader>

                    <form @submit.prevent="submitEdit" class="space-y-6 mt-4">
                        <div class="space-y-4">
                            <div>
                                <label class="block text-sm font-medium mb-2">
                                    Titel *
                            ...
                            <div class="flex items-center gap-2">
                                <input
                                    ...
                                    class="w-4 h-4 text-primary bg-background border-sidebar-border/70 rounded focus:ring-primary"
                                />
                                <label for="edit-published" class="text-sm">
                                    Artikel veröffentlichen
                                </label>
                            </div>
                        </div>
                        ...
                        <DialogFooter>
                            ...
                            <button
                                ...
                                class="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                                {{ form.processing ? 'Wird gespeichert...' : 'Aktualisieren' }}
                            </button>
                        </DialogFooter>
                    </form>
                </DialogScrollContent>
            ...
            <div class="relative overflow-hidden rounded-xl border border-sidebar-border/70 dark:border-sidebar-border bg-card p-6">
                ...
                <div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                    <div
                        ...
                        :key="request.id"
                        class="bg-card rounded-lg border border-sidebar-border/70 p-4 hover:bg-muted/50 transition-colors flex flex-col"
                    >
                        <div class="flex justify-between items-start mb-3">
                            <div class="flex-1 min-w-0">
                                <h3 class="text-base font-semibold text-foreground mb-1 truncate" :title="request.title || 'Unbenannt'">
                                ...
                                <div class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                                    ...
                                    <span class="flex items-center gap-1">
                                        ...
                                        <span class="truncate max-w-[100px]">{{ request.requester_name }}</span>
                                    </span>
                                </div>
                            </div>
                            <span
                                :class="getStatusClass(request.status)"
                                class="px-2 py-0.5 text-xs rounded-lg font-semibold border shrink-0 ml-2"
                            >
                ...
                <Link
                                    ...
                                    target="_blank"
                                    class="text-primary hover:underline truncate inline-block max-w-full"
                        ...
                        <div class="flex flex-wrap gap-2 mt-auto">
                            <button
                                ...
                                class="px-2 py-1 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg transition-colors text-xs flex items-center gap-1"
                            >
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                            ...
                            <template v-if="request.status === 'pending'">
                                <button
                                    ...
                                    class="px-2 py-1 bg-green-500/10 hover:bg-green-500/20 text-green-500 rounded-lg transition-colors text-xs flex items-center gap-1"
                                >
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <button
                                    ...
                                    class="px-2 py-1 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg transition-colors text-xs flex items-center gap-1"
                                >
                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
